...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData& meshData)
{
	...
	for(UINT i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		float theta = 0.f;//MathHelper::AngleFromXY(
...
void GeometryGenerator::loadFile()
{
	...
	std::vector<VERTEX3D> vertices(vcount);
	for (UINT i = 0; i < vcount; ++i)
	{
		fin >> vertices[i].vPos.x >> vertices[i].vPos.y >> vertices[i].vPos.z;
		fin >> vertices[i].vNormal.x >> vertices[i].vNormal.y >> vertices[i].vNormal.z;
	}
	...
	int mSkullIndexCount = 3 * tcount;
	std::vector<UINT> indices(mSkullIndexCount);// 스크린 스페이스 데칼
	...
	GET_SINGLE(CResourceManager)->CreateMesh(MESH_TYPE::MT_STATIC, "skull", D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST, 
		&vertices[0], sizeof(VERTEX3D), vcount, D3D11_USAGE_IMMUTABLE,
		 &indices[0], sizeof(UINT), mSkullIndexCount, D3D11_USAGE_IMMUTABLE, DXGI_FORMAT_R32_UINT);
}
