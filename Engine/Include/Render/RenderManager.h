...
typedef struct _tagMatrixies
{
	Matrix	matWV;
	Matrix	matWVP;
	Matrix	matLightWVP;
}MATRIXIES, *PMATRIXIES;
...
private:
	...
	std::unordered_map<size_t, class CRenderInstance*>		m_mapInstance3D[static_cast<int>(LAYER_TYPE::END)];
	class CInstancing* m_TileInst;
	std::list<class CLayer*>				m_LayerList[static_cast<int>(LAYER_TYPE::END)];
	std::unordered_map<std::string, class CMRT*>	m_mapMRT;
	std::list<class CPostProcess*>	m_PostProcessList;
	class CGraphicShader* m_pFullShader;
	class CGraphicShader* m_pLightAccShader;
	class CGraphicShader* m_pLightCombine;
	class CMesh2D* m_pMesh;
#ifdef _DEBUG
	Matrix			m_DebugWVP;
	class CGraphicShader* m_pDebugShader;
#endif
...
private:
	...
	void RenderUILayer(float fTime);
...
public:
	bool AddBlendDesc(const std::string& strTag, bool bEnable = true, D3D11_BLEND eSrc = D3D11_BLEND_SRC_ALPHA,
		...
		UINT8 iWrite = D3D11_COLOR_WRITE_ENABLE_ALL);
...
public:
	class CMRT* FindMRT(const std::string& strKey);
	class CMRT* CreateMRT(const std::string& strKey);
	void ClearTarget(const std::string& strKey);
	void SetTarget(const std::string& strKey);
	void ResetTarget(const std::string& strKey);
	void SetShader(const std::string& strKey, int iRegister, int iType = static_cast<int>(SHADER_CBUFFER_TYPE::CBUFFER_PIXEL));
	void ResetShader(const std::string& strKey, int iRegister, int iType = static_cast<int>(SHADER_CBUFFER_TYPE::CBUFFER_PIXEL));

public:
	bool CreateMultiTargetView(const std::string& strKey, int iWidth, int iHeight, DXGI_FORMAT eFmt);
	class CMultiRenderTarget* FindMultiRenderTarget(const std::string& strKey);
	void SetMultiTarget(const std::string& strKey);
	void ResetMultiTarget(const std::string& strKey);
	void AddTarget(const std::string& strKey, const std::string& strMRT);
	void ClearMultiTarget(const std::string& strKey);
...
private:
	class CRenderInstance* FindInstance3D(class CMesh* pMesh,
		class CShader* pShader, class CMaterial* pMtrl, int iLayer);
	size_t GetKey(class CMesh* pMesh, class CShader* pShader, class CMaterial* pMtrl)	const;
