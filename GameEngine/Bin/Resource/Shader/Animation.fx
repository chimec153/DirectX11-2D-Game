...
StructuredBuffer<AniFrame>	g_pTrans	:	register(t13);
StructuredBuffer<matrix>	g_pOffset	:	register(t14);
StructuredBuffer<AniFrame>	g_pNextFrames	:	register(t15);

RWStructuredBuffer<matrix>	g_pOutput	:	register(u0);
RWStructuredBuffer<matrix>	g_pBoneMatrix	:	register(u1);

[numthreads(256,1,1)]
void CS(uint3 ID	:	SV_DispatchThreadID)
{	
	AniFrame tFrm = g_pTrans[g_iAniFrameMax * ID.x + g_iFrame];

	int iNext = g_iFrame + 1;

	if (iNext >= g_iAniFrameMax)
		iNext = 0;

	AniFrame tNextFrm = g_pTrans[g_iAniFrameMax * ID.x + iNext];

	Matrix matBone = (Matrix)0.f;

	float3 vPos = lerp(tFrm.vPos.xyz, tNextFrm.vPos.xyz, g_fRatio);
	float3 vScale = lerp(tFrm.vScale.xyz, tNextFrm.vScale.xyz, g_fRatio);
	float4 vRot = QuternionLerp(tFrm.vRot, tNextFrm.vRot, g_fRatio);

	if (g_iChange)
	{
		AniFrame tChange = g_pNextFrames[g_iChangeFrameMax * ID.x];

		vPos = lerp(vPos, tChange.vPos.xyz, g_fChangeRatio * 5.f);
		vScale = lerp(vScale, tChange.vScale.xyz, g_fChangeRatio * 5.f);
		vRot = QuternionLerp(vRot, tChange.vRot, g_fChangeRatio * 5.f);
	}

	MatrixAffineTransformation(float4(vScale,0.f), float4(0.f, 0.f, 0.f, 1.f), vRot, float4(vPos,1.f),matBone);

	Matrix matOffset = transpose(g_pOffset[ID.x]);
	g_pBoneMatrix[ID.x] = transpose(matBone);
	g_pOutput[ID.x + g_iInstIndex * g_iBoneCount] = mul(matOffset, matBone);
}