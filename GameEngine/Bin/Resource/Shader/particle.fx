	...
	inout TriangleStream<GS_OUTPUT_PARTICLE> stream)
{
	...
	if (!gParticleInput[iInstID].iLive)
		return;
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		float4 vScaled = mul(float4(g_vWorldPos[i].xyz, 1.f), mat);
...
PS_OUTPUT_COLOR PS(GS_OUTPUT_PARTICLE input) : SV_TARGET
{
	...
	float fCurrentDepth = input.vProjPos.w;
	...
	float fDepth = g_DepthTexture.Sample(g_sPoint, vUV).w;

	float fOutput = 1.f;

	if (fDepth != 0.f)
	{
		clip(fDepth - fCurrentDepth);

		fOutput = saturate((fDepth - fCurrentDepth) * 0.05f);
	}

	output.vColor.a *= fOutput;
